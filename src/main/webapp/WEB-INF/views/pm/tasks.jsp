          ...
          <main class="col-md-9 ms-sm-auto col-lg-10 px-4 py-3">
            ...
            <div class="d-flex gap-2 mt-3 mb-2">
              <form class="d-flex gap-2" action="${pageContext.request.contextPath}/pm/tasks" method="get">
                <select name="moduleId" class="form-select form-select-sm" style="max-width:220px">
                  <option value="">All Modules</option>
                  <c:forEach var="m" items="${modules}">
                    <option value="${m.moduleId}" ${param.moduleId==m.moduleId || selectedModule !=null &&
                      selectedModule.moduleId==m.moduleId ? 'selected' : '' }>${m.moduleName} (${m.project != null ?
                ...
                <a href="${pageContext.request.contextPath}/pm/tasks" class="btn btn-outline-secondary btn-sm">Reset</a>
              </form>
              <a href="${pageContext.request.contextPath}/pm/tasks/add<c:if test='${selectedModule != null}'>?moduleId=${selectedModule.moduleId}</c:if>"
                class="btn btn-primary btn-sm ms-auto"><i class="bi bi-plus-lg me-1"></i>Add Task</a>
            </div>
                  ...
                  <tbody>
                    <c:forEach var="t" items="${tasks}">
                      <tr>
                        <td>${t.taskId}</td>
                        <td>${t.title}</td>
                        <td><span class="badge bg-${t.status == 'DONE' ? 'success' : 'primary'}">${t.status != null ?
                            t.status : 'OPEN'}</span></td>
