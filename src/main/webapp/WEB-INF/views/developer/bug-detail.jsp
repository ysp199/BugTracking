            ...
            <div class="table-card mt-3 p-4">
              ...
              <c:if test="${not empty bug.attachment}">
                <div class="mt-3">
                  ...
                  <a href="${bug.attachment}" target="_blank" class="btn btn-sm btn-outline-info mt-1"><i
                      class="bi bi-paperclip"></i> View Attachment</a>
                </div>
              </c:if>
              <div class="row mb-3">
                ...
                <div class="col"><strong>Reported by:</strong> ${bug.reportedBy != null ? bug.reportedBy.firstName :
                  '-'}</div>
              </div>
              <c:if test="${bug.status != 'CLOSED' && bug.status != 'VERIFIED'}">
                <hr />
                <h6>Update Status</h6>
                <form action="${pageContext.request.contextPath}/developer/bugs/${bug.bugId}/status" method="post">
                  ...
                  <textarea name="notes" class="form-control form-control-sm mt-2" rows="2"
                    placeholder="Add resolution notes or comments..."></textarea>
                </form>
              </c:if>
              <hr />
              <h6>History</h6>
